...
class Client {
  ...
  async initialize() {
    ...
    this.sock = makeWASocket({ auth: state, printQRInTerminal: true, logger: pino({ level: 'silent' }) });
    ...
    this.sock.ev.on('messages.upsert', async m => {
      if (this.events['message']) {
        for (let msg of m.messages) {
          ...
          const mappedMsg = { from: msg.key.remoteJid, to: 'me', body: text, id: { remote: msg.key.remoteJid } };
